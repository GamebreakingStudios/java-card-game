...
class PlayerTest {
	...
	void test_can_add_card_to_hand() {
		...
		player.addCardToHand(new Card("Spade", "One"));
	...
	void test_can_replace_hand() {
		List<Card> originalHand = new ArrayList<>(Arrays.asList(
				new Card("Diamond", "King"),
				new Card("Hearts", "Three")
		));
		Player player = new Player("Test Player");
		player.replaceHand(originalHand);

		assertThat(player.getHand().size()).isEqualTo(2);

		List<Card> newHand = new ArrayList<>(Arrays.asList(
